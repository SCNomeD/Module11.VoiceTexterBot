        ...
        public async Task Handle(Message message, CancellationToken ct)
        {
            switch (message.Text)
            {
                case "/start":
                    ...
                    await _telegramClient.SendTextMessageAsync(message.Chat.Id, $"<b>🎙 Мой бот превращает аудио в текст.</b> {Environment.NewLine}" +
                        $"{Environment.NewLine}🔊 Можно записать голосовое сообщение, распознать и переслать другу, если лень печатать.{Environment.NewLine}", cancellationToken: ct, parseMode: ParseMode.Html, replyMarkup: new InlineKeyboardMarkup(buttons));
                    break;
                default:
